...
/*public class PercolationUF implements IPercolate{
    ...
    @Override
    public void open(int row, int col) {
        int conv =  row * myGrid.length + col;
        if (!isOpen(row, col)) {
            myGrid[row][col] = true;
            myOpenCount++;
            try {
                if (isOpen(row, col + 1)) {
                    myFinder.union(conv,  row * myGrid.length + col + 1);
                }
            ...
            try {
                if (isOpen(row + 1, col)) {
                    myFinder.union(conv, (row + 1) * myGrid.length + col);
                }
            } catch (ArrayIndexOutOfBoundsException ex) {}
            try {
                if (isOpen(row - 1, col)) {
            ...
            if (row == 0) {
                myFinder.union(conv, VTOP);
            }
            if (row == Math.sqrt(VTOP)) {
                myFinder.union(VBOTTOM, conv);
            }
        }
    }
    ...
    @Override
    public boolean isFull(int row, int col) {
        int converted = row * myGrid.length + col;
        if (inBounds(row,col)) {
            return myFinder.connected(converted, VTOP);
        } else {
